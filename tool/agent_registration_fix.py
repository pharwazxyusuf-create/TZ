...
# Add a simple Register as Agent action to the login screen.
...
# Replace ProfileGate so agents must be approved before they can enter TZ.
...
class _ProfileGateState extends State<ProfileGate> {
  ...
  Future<void> loadProfile() async {
    try {
      ...
      if (r == 'agent' && row['available'] == false) throw Exception('Your Temz Store agent account is currently inactive. Contact an administrator.');
      if (!mounted) return;
      setState(() {
        role = r;
        name = '${row['full_name'] ?? ''}';
...
# Replace the old invitation-based Agents screen with a pending/approve/reject workflow.
...
class _AgentsPageState extends State<AgentsPage> {
  ...
  Future<List<Map<String, dynamic>>> loadAgents() async {
    final rows = await db.from('profiles').select('id,full_name,email,phone,state,available,password_set,agent_approval_status').eq('role','agent').order('full_name');
        ...
        child: ListView(
          ...
          children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              ...
              if (pending > 0) Chip(label: Text('$pending pending'), avatar: const Icon(Icons.hourglass_top,size:18)),
            ]),
            const SizedBox(height:8),
            const Text('Agents register themselves. Approve them here before they can use TZ.', style: TextStyle(color:Colors.grey)),
            ...
            for (final a in rows) Card(child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ListTile(contentPadding: EdgeInsets.zero, leading: CircleAvatar(child: Text('${a['full_name'] ?? 'A'}'.trim().isEmpty ? 'A' : '${a['full_name']}'.trim()[0].toUpperCase())), title: Text('${a['full_name'] ?? 'Unnamed agent'}', style: const TextStyle(fontWeight:FontWeight.w800)), subtitle: Text('${a['email'] ?? ''}\n${a['phone'] ?? ''} • ${a['state'] ?? 'State not set'}'), isThreeLine:true, trailing: Text('${a['agent_approval_status'] ?? 'approved'}'.toUpperCase(), style: TextStyle(fontSize:11,fontWeight:FontWeight.w800,color: a['agent_approval_status']=='approved' ? green : (a['agent_approval_status']=='pending' ? orange : red)))),
              if (a['agent_approval_status'] == 'pending') Row(children: [
                Expanded(child: FilledButton.icon(onPressed: () => setApproval('${a['id']}', 'approved'), icon: const Icon(Icons.check), label: const Text('APPROVE'))),
                const SizedBox(width:8),
                Expanded(child: OutlinedButton.icon(onPressed: () => setApproval('${a['id']}', 'rejected'), icon: const Icon(Icons.close), label: const Text('REJECT'))),
              ]),
              if (a['agent_approval_status'] == 'approved') Row(children: [
                Expanded(child: OutlinedButton.icon(onPressed: () => setActive('${a['id']}', !(a['available'] == true)), icon: Icon(a['available'] == true ? Icons.person_remove : Icons.person_add), label: Text(a['available'] == true ? 'REMOVE' : 'RESTORE'))),
              ]),
            ]))),
          ],
        ),
...
class _AgentRegistrationPageState extends State<AgentRegistrationPage> {
  ...
  Future<void> register() async {
    final n=name.text.trim(); final e=email.text.trim().toLowerCase();
    if(n.isEmpty || !e.contains('@') || phone.text.trim().isEmpty || state.text.trim().isEmpty || password.text.length<8 || password.text!=confirm.text) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Complete all fields. Passwords must match and contain at least 8 characters.'))); return;
    }
    setState(()=>busy=true);
    try {
      final res=await db.auth.signUp(email:e,password:password.text,data:{'full_name':n,'phone':phone.text.trim(),'state':state.text.trim(),'registration_type':'agent'});
      await db.auth.signOut();
      if(!mounted) return;
      final needsEmailConfirmation = res.session == null;
      await showDialog<void>(context:context,builder:(_)=>AlertDialog(title:const Text('Registration submitted'),content:Text(needsEmailConfirmation ? 'Your registration has been submitted. If Temz Store email confirmation is enabled, confirm your email once. An administrator must approve your registration before you can sign in.' : 'Your registration has been submitted. An administrator must approve your registration before you can sign in.'),actions:[TextButton(onPressed:()=>Navigator.pop(context),child:const Text('OK'))]));
  ...
  Widget build(BuildContext context)=>Scaffold(appBar:AppBar(title:const Text('Register as Agent')),body:ListView(padding:const EdgeInsets.all(22),children:[
    const Logo(size:105), const SizedBox(height:10),
    const Text('Join Temz Store as an Agent',style:TextStyle(fontSize:25,fontWeight:FontWeight.w900,color:navy)),
    const SizedBox(height:6), const Text('Submit your details. A Temz Store administrator will review and approve your registration.'),
    ...
    TextField(controller:phone,keyboardType:TextInputType.phone,decoration:const InputDecoration(labelText:'Phone number',prefixIcon:Icon(Icons.phone_outlined))),const SizedBox(height:12),
    TextField(controller:state,decoration:const InputDecoration(labelText:'State',prefixIcon:Icon(Icons.location_on_outlined))),const SizedBox(height:12),
    TextField(controller:password,obscureText:hide,decoration:InputDecoration(labelText:'Create password',prefixIcon:const Icon(Icons.lock_outline),suffixIcon:IconButton(onPressed:()=>setState(()=>hide=!hide),icon:Icon(hide?Icons.visibility:Icons.visibility_off)))),const SizedBox(height:12),
    TextField(controller:confirm,obscureText:hide,decoration:const InputDecoration(labelText:'Confirm password',prefixIcon:Icon(Icons.verified_user_outlined))),const SizedBox(height:20),
...
print('Applied simple agent self-registration and admin approval workflow.')
