...
begin
  ...
  insert into public.notifications(user_id,title,body,type,order_id)
    select id,'Agent rejected order',o.order_number || ' was rejected and is ready for reassignment.','agent_rejection',p_order
...
begin
  ...
  insert into public.agent_stock(agent_id,product_id,quantity,updated_at)
  values(p_agent,p_product,p_quantity,now())
...
begin
  for r in select id,assigned_agent_id,order_number from public.orders where assigned_agent_id is not null and status not in ('Delivered','Cancelled') and last_agent_update_at < now() - interval '3 hours' loop
    insert into public.notifications(user_id,title,body,type,order_id)
    values(r.assigned_agent_id,'TZ order reminder',r.order_number || ' needs a delivery status update.','reminder',r.id);
